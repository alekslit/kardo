...
/* TODO delete:
-- таблица directions_import (для заполнения directions):
CREATE TABLE IF NOT EXISTS directions_import(
    name varchar(20)
);*/
...
/* TODO удалить:
-- таблица user_directions (связывает user и directions):
CREATE TABLE IF NOT EXISTS user_directions(
    user_id BIGINT NOT NULL,
    direction_id integer NOT NULL,
    CONSTRAINT fk_user_directions_to_users FOREIGN KEY (user_id) REFERENCES users(user_id)
    ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_user_directions_to_directions FOREIGN KEY (direction_id) REFERENCES directions(direction_id)
    ON DELETE CASCADE ON UPDATE CASCADE
);*/